...
public class ChallengeValidator {
    ...
    public EnumSet<ChallengeRequirement> validate(Challenge challenge) {
        ...
        if (challenge.getName() == null || challenge.getName().isEmpty()) missing.add(ChallengeRequirement.NAME_SET);
        if (challenge.getTime() < 0) missing.add(ChallengeRequirement.TIME_SET);
        if (challenge.getTeams() == null || challenge.getTeams().isEmpty()) missing.add(ChallengeRequirement.TEAMS_DEFINED);
        else {
